...
export function AuthForm() {
    ...
    const handleSubmit = form.handleSubmit(async (data) => {
        try {
            await signIn('nodemailer', { redirect: false, email: data.email, callbackUrl: '', })
            toast({
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <Input id="email" placeholder="Insira seu E-mail" required type="email" {...form.register('email')} />
                        </div>
